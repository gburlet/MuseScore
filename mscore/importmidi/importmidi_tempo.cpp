    ...
    namespace MidiTempo {
        ...
        void setTempo(const std::multimap<int, MTrack> &tracks, Score *score)
        {
            ...
            if (beats.empty()) {
                ...
                const double basicTempoBps = MidiTempo::findBasicTempo(tracks, true); // bps
                double tempoBps = roundToBpm(basicTempoBps * averageTempoFactor); // bps
                midiData->trackOpers.tempo.setDefaultValue(tempoBps * 60.0); // bpm
                tempoBps = midiData->trackOpers.tempo.defaultValue() / 60.0; // bps
                
                score->tempomap()->clear();         // use only one tempo marking for all score
                setTempoToScore(score, 0, tempoBps);
            }
            
            if (score->tempomap()->empty()) {
                const double defaultTempo = roundToBpm(midiData->trackOpers.tempo.defaultValue() / 60.0); // bps
                score->tempomap()->setTempo(0, defaultTempo);      // default tempo
            }
        }
